$(document).ready(function() {
        ...
        if(searchData == ''){
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    // if(res.sonodno){
                    //     info = res.message+'<b>'+res.pin+'</b> এবং সনদ নং <b>'+res.sonodno+'</b> <br/><br/><a href="'+loc+'/'+res.application+'/print_bn/'+res.sonodno+'" type="button" class="btn btn-info" target="_blank">সনদটি বাংলায় প্রিন্ট করুন</a> <br/><br/> <a href="'+loc+'/'+res.application+'/print_en/'+res.sonodno+'" type="button" class="btn btn-info" target="_blank">সনদটি ইংরেজিতে প্রিন্ট করুন</a>';
                    //     sweetAlert(res.status, info);
                    // }else if(res.tracking){
                    //     info = res.message+'<b>'+res.pin+'</b> এবং ট্র্যাকিং নং <b>'+res.tracking+'</b> <br/><br/><a href="'+loc+'/'+res.application+'/preview/'+res.tracking+'" type="button" class="btn btn-info" target="_blank">আবেদনটি প্রিন্ট করুন</a>';
                    //     sweetAlert(res.status, info);
                    ...
                    // }else{
                        ...
                        if(res.gender == 2 && res.marital_status == 2){
                            $('#genderError_0').removeAttr('style');
                            $('#genderErrorField_0').html('');
                        ...
                        // $('#form-data').attr('action', loc+'/premises');

                        $("#image-0").attr('src', asset_url+'/images/application/'+res.photo);

                        $("#gender_"+res.gender).attr('checked', true);
                        $(".gender").attr('disabled', true);
                        // $(".wrap").css('opacity', '.2');

                        $('#marital_status_0').val(res.marital_status);
                        $('#husband_name_en_0').val(res.husband_name_en);
                        $('#husband_name_bn_0').val(res.husband_name_bn);

                        // present
                        $('#present_village_en_0').val(res.present_village_en);
                        $('#present_village_bn_0').val(res.present_village_bn);

                        $('#present_rbs_en_0').val(res.present_rbs_en);
                        $('#present_rbs_bn_0').val(res.present_rbs_bn);

                        $('#present_holding_no_0').val(res.present_holding_no);
                        $('#present_ward_no_0').val(res.present_ward_no);

                        // present district
                        $('#present_district_id_0').append('<option value="'+res.present_district_id + '" selected="selected">'+res.present_district_name_en + '</option>');

                        $('#present_district_0').val(res.present_district_name_bn);

                        $('#present_upazila_id_0').html('<option value="'+res.present_upazila_id + '" selected="selected">' + res.present_upazila_name_en + '</option>');

                        $('#present_upazila_0').val(res.present_upazila_name_bn);

                        $('#present_postoffice_id_0').html('<option value="'+res.present_postoffice_id + '" selected="selected">' + res.present_postoffice_name_en + '</option>');

                        $('#present_postoffice_0').val(res.present_postoffice_name_bn);

                        $('#addressCheck-0').remove();

                        $('#permanent_district_id_0').append('<option value="' + res.permanent_district_id + '" selected="selected">' + res.permanent_district_name_en + '</option>');

                        $('#permanent_district_id_0').prop('disabled', true);

                        $('#permanent_district_0').val(res.permanent_district_name_bn);

                        $('#permanent_upazila_id_0').html('<option value="'+res.permanent_upazila_id + '" selected="selected">' + res.permanent_upazila_name_en + '</option>');

                        $('#permanent_upazila_id_0').prop('disabled', true);

                        $('#permanent_upazila_0').val(res.permanent_upazila_name_bn);

                        $('#permanent_postoffice_id_0').html('<option value="' + res.permanent_postoffice_id + '" selected="selected">' + res.permanent_postoffice_name_en + '</option>');

                        $('#permanent_postoffice_id_0').prop('disabled', true);

                        $('#permanent_postoffice_0').val(res.permanent_postoffice_name_bn);
                    // }
    ...
    function resetApplicationForm(){
        // skip ids
        var skip_ids = ["present_district_0", "present_upazila_0", "present_postoffice_0", "permanent_district_0", "permanent_upazila_0", "permanent_postoffice_0"];
        ...
        $("*").each(function(){
            if(this.id && this.id != "type_of_organization"){
                if(this.id == 'gender' || this.id == 'gender_0' || this.id == 'gender_1' || this.id == 'gender_2'){
                    ...
                    var indexOfItem = skip_ids.indexOf(this.id);
                    ...
                    if(indexOfItem === -1){
                        ...
                        // district
                        if(indexOfItem == 0 || indexOfItem == 3){
                            document.getElementById(this.id).value = 'জেলা';
                        } 
                        // upazila
                        else if (indexOfItem == 1 || indexOfItem == 4){
                            document.getElementById(this.id).value = 'উপাজেলা';
                        }
                        // postoffice
                        else if (indexOfItem == 2 || indexOfItem == 5){
                            document.getElementById(this.id).value = 'পোষ্ট অফিস';
                        }
                    }
