...
var warish_table, fiscal_year_id, from_date, to_date;
 ...
 function applicant_list_search(){
    ...
    $("#warish_applicant_table").dataTable().fnSettings().ajax.data.fiscal_year_id = fiscal_year_id;
 ...
 //get account list
            ...
            success:function(response){
                ...
                if(response['status'] == "success"){
                    ...
                    response['data'].forEach(function(data) {
                        ...
                        if (data.id == 28)
                            option += '<option value ="' + data.id + '"> ভূমিহীন সনদ</option>';
    ...
    function generate_save()
    {
        ...
        var investigator_name_bn = $("#investigator_name_bn").val();
        var investigator_name_en = $("#investigator_name_en").val();
        var element = document.getElementById('investigator_name_bn');

        if(investigator_name_bn == ''){
            ...
            swal({
                ...
                preConfirm: function() {
                    ...
                    data: {
                        ...
                        investigator_name_bn: investigator_name_bn,
                        investigator_name_en: investigator_name_en,
                    ...
                    success: function(response) {
                        ...
                        swal({
                            ...
                            showConfirmButton: true,
                            confirmButtonText: '<a href="'+warish_bangla_sonod_url+'/'+response.sonod_no+'" target="_blank">প্রিন্ট</a>',
                            closeOnConfirm: true,
     ...
     warish_certi_table = $('#warish_certificate_table').DataTable({
         ...
         columns: [{
             ...
             },
             {
                data: null,
                render:function(data, type, row) {
                    return "<a class='link_color' href='edit/"+data.tracking+"/"+data.sonod_no+"'>"+data.name_bn+"</a>";
                }
            },
             { data: "father_name_bn" },
             {
             ...
             },
             {
                 ...
                 render: function(data, type, row, meta) {
                    ...
                    if($('#edit').val()){
                        edit = "<a  href='edit/"+data.tracking+"/"+data.sonod_no+"'><p class='btn btn-sm btn-warning' >এডিট</p></a> ";
                    }
                    if($('#invoice').val()){
...
//this is for warish certificate list search
 function certificate_list_search() {
    ...
    to_date = $("#filter_to_date").val();
     ...
     $("#warish_certificate_table").dataTable().fnSettings().ajax.data.fiscal_year_id = fiscal_year_id;
 ...
 //====warish re-generate====//
    function warish_regenerate(row_index){
        ...
        $("#investigator_name_bn").val(row_data.investigator_name_bn);
        $("#investigator_name_en").val(row_data.investigator_name_en);
